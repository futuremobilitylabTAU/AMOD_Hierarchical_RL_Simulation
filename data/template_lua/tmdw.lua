...
local beta_cost_AMOD = (beta_cost_car + beta_cost_PT)/2
local beta_cost_AMOD_Pool = (beta_cost_car + beta_cost_PT)/2
...
local beta_cons_AMOD_Pool = beta_cons_taxi+beta_cost_AMOD*0.65 
...
-- 4 for shared2; 5 for shared3+; 6 for motor; 7 for walk; 
-- 8 for AMOD; 9 for AMOD_Pool; 10 for bike; 11 for MRT
...
--	end
	...
	for i=1,917 do
		...
		utility[V_counter] = beta_cons_AMOD  + beta_cost_AMOD*cost_AMOD [i]+ 
	...
	for i=1,917 do
		...
		utility[V_counter] = beta_cons_AMOD_Pool + beta_cost_AMOD_Pool* cost_AMOD_Pool[i] +
