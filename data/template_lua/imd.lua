...
local beta_cons_AMOD_Pool = beta_cons_taxi                          
...
local beta_cbd_AMOD = (beta_cbd_notdrive+beta_cbd_PT)/2
local beta_cbd_AMOD_Pool = (beta_cbd_notdrive+beta_cbd_PT)/2
...
local function computeUtilities(params,dbparams)
	...
	local cost_AMOD_first = {}
	local cost_AMOD_second = {}

	local cost_AMOD_Pool_first = {}
	local cost_AMOD_Pool_second = {}
	...
	local tt_SMS_second = {}
	local tt_AMOD_first = {}
	local tt_AMOD_second = {}
	local tt_AMOD_Pool_first = {}
	local tt_AMOD_Pool_second = {}
	local tt_bike_first = {}
	...
	for i =1,num_zone do
		...
		--AMOD
		cost_AMOD_first[i] = cost_taxi_first[i]*0.5
		tt_AMOD_first[i] = tt_drive1_first[i]

		--AMOD Pool
		cost_AMOD_Pool_first[i] = 0.7*cost_AMOD_first[i]
		tt_AMOD_Pool_first[i]= tt_AMOD_first[i] * 1.35 + 5/60
		--tt_AMOD_Pool_first[i]= dbparams:tt_car_first(i) * 1.35 + 5/60
	...
	for i =1,num_zone do
		...
		--AMOD
		cost_AMOD_second[i] = cost_taxi_first[i]*0.5
		tt_AMOD_second[i] = tt_drive1_second[i]

		--AMOD Pool
		cost_AMOD_Pool_second[i] = 0.7*cost_AMOD_second[i]
		tt_AMOD_Pool_second[i]= tt_AMOD_second[i] * 1.35 + 5/60
		--tt_AMOD_Pool_second[i]= dbparams:tt_car_second(i) * 1.35 + 5/60
...
--    Alternatives where second trip is fixed to tour mode (1, N_zones*Nmodes)
	...
	for i=1,num_zone do
		...
		utility[V_counter] = beta_cons_AMOD + beta_cost * cost_AMOD_first[i]+ 
		beta_tt_AMOD* tt_AMOD_first[i] +
		(beta_log_work * work + beta_log_edu * edu + 
		beta_log_shop * shop + beta_log_other * other) * 
		log(area[i] + exp(beta_employment) * employment[i]+exp(beta_shop)*SHOP[i]) + 
		(beta_cbd_work * work + beta_cbd_edu * edu + beta_cbd_shop * shop + 
		beta_cbd_other * other+beta_cbd_AMOD) * central_dummy[i]
	end

	--utility function for AMOD_Pool 1-917

	for i=1,num_zone do

		V_counter = V_counter +1

		utility[V_counter] = beta_cons_AMOD_Pool + beta_cost * cost_AMOD_Pool_first[i]+
		beta_tt_AMOD_Pool* tt_AMOD_Pool_first[i] +
		(beta_log_work * work + beta_log_edu * edu + 
		beta_log_shop * shop + beta_log_other * other) * 
		log(area[i] + exp(beta_employment) * employment[i]+exp(beta_shop)*SHOP[i]) + 
		(beta_cbd_work * work + beta_cbd_edu * edu + beta_cbd_shop * shop + 
		beta_cbd_other * other+beta_cbd_AMOD_Pool) * central_dummy[i]
	end
	...
	for i=1,num_zone do
		...
		utility[V_counter] = beta_cons_AMOD + beta_cost * cost_AMOD_second[i]+ 
		beta_tt_AMOD* tt_AMOD_second[i] +
		(beta_log_work * work + beta_log_edu * edu + 
		beta_log_shop * shop + beta_log_other * other) * 
		log(area[i] + exp(beta_employment) * employment[i]+exp(beta_shop)*SHOP[i]) + 
		(beta_cbd_work * work + beta_cbd_edu * edu + beta_cbd_shop * shop + 
	...
	for i=1,num_zone do
		...
		utility[V_counter] = beta_cons_AMOD_Pool + beta_cost * cost_AMOD_Pool_second[i]+
		beta_tt_AMOD_Pool* tt_AMOD_Pool_second[i] +
		(beta_log_work * work + beta_log_edu * edu + 
		beta_log_shop * shop + beta_log_other * other) * 
		log(area[i] + exp(beta_employment) * employment[i]+exp(beta_shop)*SHOP[i]) + 
		(beta_cbd_work * work + beta_cbd_edu * edu + beta_cbd_shop * shop + 
...
local function computeAvailabilities(params,dbparams)
    ...
    --set AMOD to available
    for i = (917*7+1),917*8 do
        availability[i] = 1
    end
    for i = (917*18+1),917*19 do
        availability[i] = 1
    end

    --set AMOD_Pool to available
